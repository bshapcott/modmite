...
#if CH_DEV
//____________________________________________________________________________
/// - special handling required for virtual tables
/// @todo - improve inefficient implementation of query, even if this is just
///         for developer mode
int sql_virtual_pk(Transaction *xn, char const *key) {
  char buf[256];
  char const *col;
  sqlite3_stmt *stmt;
  int pk = 0,
    // - WON'T find virtual table columns
    rc = sqlite3_table_column_metadata(xn->db, NULL, xn->name,
    key, NULL, NULL, NULL, &pk, NULL);
  if (rc != SQLITE_OK) {
    // - WILL find virtual table columns
    sprintf(buf, "PRAGMA table_info(%s)", xn->name);
    sqlite3_prepare_v2(xn->db, buf, -1, &stmt, NULL);
    while (SQLITE_ROW == sqlite3_step(stmt)) {
      col = sqlite3_column_text(stmt, 1);
      if (stricmp(col, key) == 0) {
